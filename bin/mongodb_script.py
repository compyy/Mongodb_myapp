...
last_id = 0
config_file = '/opt/splunk/etc/apps/Mongodb_myapp/config.json'
...
def get_config(config_file_name):
    config = open(config_file_name).read()
...
def check_file(config_file_name):
    last_id_filepath = '/opt/splunk/etc/apps/Mongodb_myapp/last_id'  # user supplies correct path
...
def connect(data, last_id, last_id_filepath):
    try:
        client = pymongo.MongoClient(data['server'])
        client.admin.authenticate(data["user"], data["password"], mechanism='SCRAM-SHA-1')
        max_id = []
        this_last_id = 0
        database = data["database"]
        collection = data["collection"]
